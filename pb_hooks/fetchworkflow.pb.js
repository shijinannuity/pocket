...
routerAdd("GET", "/fetchworkflow", (c) => {
	...
	$app.dao().db().newQuery("SELECT wf.id as wf_row_id,wf.wfname as wf_name,wf.user_id,wf.description,wf.isactive,( SELECT json_group_array( json_object('wf_app_row_id', workflow_app.id,'app_order', workflow_app.app_order, 'app', (SELECT json_object('table_id',apps.id,'app_id',apps.app_id,'name',apps.name,'logo',apps.file,'internalurl',apps.internalurl,'internalport',apps.internalport,'company_id',apps.company_id) FROM apps WHERE apps.id = workflow_app.app AND apps.isactive = TRUE))) FROM workflow_app WHERE workflow_app.wf_id = wf.id ORDER BY workflow_app.app_order) as wf_apps FROM workflow as wf WHERE wf.user_id = {:uid} ORDER BY -wf.isactive,-wf.updated").bind({ "uid": userid }).all(result)
